<template>
  <div class="layout">
    ...
    <!--      </div>-->
    <!--      <div class="nav">-->
    <!--        <mu-flat-button v-for="tab in tabs" :key="tab.name" :label="tab.name" class="tab demo-flat-button" :icon="tab.icon" primary background-color="#FFFFFF" @click="tabfun(tab.fun)" />-->
    <!--        <mu-flat-button :label="chooseColorBtn" class="tab demo-flat-button" icon="color_lens" primary background-color="#FFFFFF" :color="color.hex" @click="setColor()" />-->
    <!--        <a v-show="false" ref="download" href="javascript:void(0);" download="picture.png" />-->
    <!--      </div>-->
    <!--      <PhotoshopPicker v-if="ischoosecolor" v-model="color" style="position:absolute;z-index:10;top:56px;" @ok="setColor()" @cancel="falseColor()" />-->
    <!--    </div>-->
    <div class="content">
      <div class="content-left">
        ...
        <div v-for="tool in tools" :key="tool.name">
          <el-button style="margin-left: 20%;width:60%;margin-top: 15px" :type="tool.ischoose?'success':'primary'"
                     @click="drawType(tool)" round>{{ tool.name }}
          </el-button>
        </div>
        <!--          <mu-menu v-for="tool in tools" :key="tool.name">-->
        <!--            <mu-menu-item :title="tool.name" :left-icon="tool.icon" :class="{'selected':tool.ischoose}" @click="drawType(tool)" />-->
        <!--          </mu-menu>-->
        <!--        </mu-paper>-->
      </div>
      <div class="content-right">
        <el-alert
          title="您可拖拽图片进入画板，亦可保存图像。后续会增加分享学生端。"
          style="margin-top: 10px;margin-bottom: 10px"
          type="success"
          show-icon>
        </el-alert>
        <div class="body" :style="{width:canvasSize.width + 'px',height: canvasSize.height + 'px' }">
          <canvas id="canvas" ref="canvas" :style="{cursor:curcursor}"/>
          <canvas id="canvas_bak" ref="canvas_bak" :style="{cursor:curcursor}"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    // this.$alert('欢迎使用课堂互动工具演示白板，使用时建议将菜单栏折叠，可获得更高的精准度。白板支持拖拽素材，以及导出图片！', '使用说明', {
  ...
  methods: {
    ...
    cancel() {
      ...
      const image = new Image()
      const index = this.cancelList.length - 1 - this.cancelIndex
      const url = this.cancelList[index]
      image.src = url
      image.onload = () => {
        this.context.drawImage(
          image,
    ...
    // 初始化拖入效果
    initDrag() {
      const dragDiv = document.getElementById('canvas_bak')
